#!/bin/bash
# Scale study: cache OFF, 32-256 clients, 8 nodes
NPARTICLES=8388608
STEPS=5
SLEEP_TIME=20
...
cd $BUILD_DIR
cmake .. -DPDC_SERVER_CACHE=OFF > /dev/null && make -j${NPROC} 2>&1 | tail -3
...
run_test() {
    ...
    sleep 2
    echo "=== cache=0 servers=${n_servers} clients=${n_clients} strategy=${strategy} ==="
...
echo "### cache=OFF: fixed 8 servers, scale clients 32-256 ###"
