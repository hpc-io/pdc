...
main(int argc, char **argv)
{
    ...
    transfer_request = PDCregion_transfer_create(data + BUF_LEN / 2, PDC_WRITE, obj1, reg, reg_global);
    ...
    for (i = 0; i < BUF_LEN / 2; ++i) {
        if (data_read[i] != i + BUF_LEN / 4) {
            printf("i = %d, wrong value %d!=%d @ line  %d!\n", i, data_read[i], i + BUF_LEN / 4, __LINE__);
            ret_value = 1;
            break;
        }
    }
    if (ret_value == 0) {
        printf("Successfuly verified read 0\n");
    }
    ...
    if (ret_value == 0) {
        printf("Successfuly verified read 1\n");
    }
