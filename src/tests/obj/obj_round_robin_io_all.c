...
main(int argc, char **argv)
{
    ...
    TASSERT((transfer_request[0] =
                 PDCregion_transfer_create(mydata[0], PDC_WRITE, obj1, local_region, global_region)) != 0,
    ...
    TASSERT((transfer_request[1] =
                 PDCregion_transfer_create(mydata[1], PDC_WRITE, obj2, local_region, global_region)) != 0,
            "Call to PDCregion_transfer_create succeeded", "Call to PDCregion_transfer_create failed");
    TASSERT(PDCregion_transfer_start_all(transfer_request, WRITE_REQ_SIZE) >= 0,
            "Call to PDCregion_transfer_start_all succeeded", "Call to PDCregion_transfer_start_all failed");
    TASSERT(PDCregion_transfer_wait_all(transfer_request, WRITE_REQ_SIZE) >= 0,
            "Call to PDCregion_transfer_wait_all succeeded", "Call to PDCregion_transfer_wait_all failed");
    TASSERT(PDCregion_transfer_close(transfer_request[0]) >= 0, "Call to PDCregion_transfer_close succeeded",
            "Call to PDCregion_transfer_close failed");
    TASSERT(PDCregion_transfer_close(transfer_request[1]) >= 0, "Call to PDCregion_transfer_close succeeded",
            "Call to PDCregion_transfer_close failed");
...
// Wait for all processes to finish their object creation
    ...
    for (i = 1; i < size; ++i) {
        ...
        TASSERT((obj1_list[i - 1] = PDCobj_open(obj_name1, pdc)) != 0, "Call to PDCobj_open succeeded",
        ...
        if (transfer_request[(i - 1)] == 0) {
            LOG_ERROR("PDCregion_transfer_create for read obj2 failed\n");
            ret_value = 1;
        }
        ...
        TASSERT((transfer_request[(i - 1) + size - 1] = PDCregion_transfer_create(
                     data_read[(i - 1) + size - 1], PDC_READ, obj2, local_region, global_region)) != 0,
                "Call to PDCregion_transfer_create succeeded", "Call to PDCregion_transfer_create failed");
    ...
    TASSERT(PDCregion_transfer_start_all(transfer_request, (size - 1) * 2) >= 0,
            "Call to PDCregion_transfer_start_all succeeded", "Call to PDCregion_transfer_start_all failed");
    TASSERT(PDCregion_transfer_wait_all(transfer_request, (size - 1) * 2) >= 0,
            "Call to PDCregion_transfer_wait_all succeeded", "Call to PDCregion_transfer_wait_all failed");

    for (i = 1; i < size; ++i) {
        target_rank = (rank + i) % size;

        TASSERT(PDCregion_transfer_close(transfer_request[i - 1]) >= 0,
                "Call to PDCregion_transfer_close succeeded", "Call to PDCregion_transfer_close failed");
        TASSERT(PDCregion_transfer_close(transfer_request[i - 1] + size - 1) >= 0,
                "Call to PDCregion_transfer_close succeeded", "Call to PDCregion_transfer_close failed");

        for (j = 0; j < (int)(my_data_size * type_size); ++j) {
            if (data_read[(i - 1)][j] != (char)(j + target_rank)) {
                TGOTO_ERROR(TFAIL, "Rank %d, i = %d, j = %d, wrong value %d!=%d\n", rank, i, j,
                            data_read[(i - 1)][j], (char)(j + target_rank));
            }
        }

        for (j = 0; j < (int)(my_data_size * type_size); ++j) {
            if (data_read[(i - 1) + size - 1][j] != (char)(j + target_rank * 5 + 3)) {
                TGOTO_ERROR(TFAIL, "Rank %d, i = %d, j = %d, wrong value %d!=%d", rank, i, j,
                            data_read[(i - 1) + size - 1][j], (char)(j + target_rank * 5 + 3));
            }
        }
        ...
        TASSERT(PDCobj_close(obj1_list[i - 1]) >= 0, "Call to PDCobj_close succeeded",
                "Call to PDCobj_close failed");
        TASSERT(PDCobj_close(obj2_list[i - 1]) >= 0, "Call to PDCobj_close succeeded",
                "Call to PDCobj_close failed");
    }
