...
int main(int argc, char **argv)
{
    ...
    // create a pdc
    pdc_id = PDC_init("pdc");
    if(pdc_id <= 0)  printf("PDC_init failed @ line  %d!\n", __LINE__);

    // create a container property
    cont_prop = PDCprop_create(PDC_CONT_CREATE, pdc_id);
    if(cont_prop <= 0) printf("Fail to create container property @ line  %d!\n", __LINE__);

    // create a container
    cont_id = PDCcont_create("c1", cont_prop);
    if(cont_id <= 0) printf("Fail to create container @ line  %d!\n", __LINE__);
    ...
    ret = PDCreg_obtain_lock(obj_ix, region_ix, WRITE, NOBLOCK);
    if (ret != SUCCEED)
    ...
    ret = PDCreg_release_lock(obj_ix, region_ix, WRITE);
    if (ret != SUCCEED)
