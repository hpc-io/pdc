...
size_t
cortad_avg_func(short *stencil[4], float *out, size_t elements)
{
    int k;
 ...
 */
size_t
neon_stencil(pdcid_t iterIn, pdcid_t iterOut, iterator_cbs_t *callbacks)
{
    short *dataIn     = NULL;
    ...
    if ((blockLengthIn = (*callbacks->getNextBlock)(iterIn, (void **)&dataIn, dimsIn)) == 0)
        ...
        printf("Total # Slices = %ld, Size of each slice = %ld elements = (%ld x %ld)\n", number_of_slices,
               blockLengthIn, dimsIn[2], dimsIn[1]);
...
size_t
myfunc3(float *stencil[3], float *out, size_t dims[2])
{
    ...
    for (k = 0; k < elements; k++) {
        ...
        if (k == 0) /* self       +    above      +   below       +    left           + right */
            ...
            out[k] = (stencil[1][last] + stencil[0][last] + stencil[2][last] + stencil[1][last - 1] +
                      stencil[1][last]) *
                     5;
        /* The normal case is last */
        else
            out[k] =
                (stencil[1][k] + stencil[0][k] + stencil[2][k] + stencil[1][k - 1] + stencil[1][k + 1]) * 5;
    }
 ...
 */
size_t
msi_analysis_func(float *stencil[3], float *out, size_t elements, size_t row_length)
{
    float  my_value;
    size_t k;
...
size_t
openmsi_stencil(pdcid_t iterIn, pdcid_t iterOut, iterator_cbs_t *callbacks)
{
    float *dataIn     = NULL;
    ...
    for (k = 0; k < number_of_slices; k++) {
        if ((blockLengthOut = (*callbacks->getNextBlock)(iterOut, (void **)&dataOut, dimsOut)) > 0) {
            if ((*callbacks->getNextBlock)(iterIn, (void **)&dataIn, dimsIn) > 0) {
                ...
                result = msi_analysis_func(stencil, dataOut, blockLengthIn, dimsIn[2]);
        }
    }
    end   = MPI_Wtime();
    total = end - start;
...
size_t
s3d_analysis_func(float *stencil[1], float *out, size_t elements, size_t row_length)
{
    size_t k;
    size_t last_row = elements - row_length;
    for (k = 0; k < elements; k++) {
...
size_t
s3d_stencil(pdcid_t iterIn, pdcid_t iterOut, iterator_cbs_t *callbacks)
{
    float *dataIn     = NULL;
