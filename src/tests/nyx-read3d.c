...
void
get_offsets_array(hid_t grp_id, void **buf, size_t *len)
{
...
void
pdc_write_offsets_array(pdcid_t pdc_id, pdcid_t cont_id, int64_t *offsets_array, size_t offsets_array_len)
{
    ...
    pdcid_t transfer_request_offsets = PDCregion_transfer_create(&offsets_array[0], PDC_WRITE, obj_offsets,
                                                                 region_offsets, regions_offsets_remote);
...
void
pdc_write_3d_box(pdcid_t pdc_id, pdcid_t cont_id, double **component_patch_array, int n_patches)
{
    pdcid_t obj_prop[13];
    ...
    for (int i = 0; i < 13; i++) {
        xfer_request[i] = PDCregion_transfer_create(&component_patch_array[i][0], PDC_WRITE, obj[i],
                                                    region[i], region_remote[i]);
...

    {
        ...
        for (int i = patch_start; i < patch_end; i++) {
            ...
            for (int component_id = 0; component_id < 13; component_id++) {
                ...
                uint64_t patch_offset = i * ngrid;
                memcpy(component_patch_array[component_id] + patch_offset, patch_component_data,
                       BOX_AXIS_DIM * BOX_AXIS_DIM * BOX_AXIS_DIM * sizeof(double));
