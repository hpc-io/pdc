...
int do_pdc_analysis_for_output(short *data, size_t dims[3], pdc_var_type_t data_type, int mpi_rank)
{
  perr_t ret = SUCCEED;
...
int read_client_data(int grank, int gsize, char *filename, char *datasetID)
{
  int i,k, errors = 0;
  int ds_ndims = 0;
...
int main(int argc, char **argv)
{
  ...
  if (filename == NULL) {
    if (temp) {
      ...
      if (mpi_rank == 0)
      {
        printf("Usage: %s <plotfile> [dataset-identifier]\n", argv[0]);
      }
    }
    goto finalize;
  }
