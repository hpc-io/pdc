...
int main(int argc, char **argv)
{
    ...
    int n_var = NUM_VAR_MAX, n_ts = 1, sleep_time = 0; 
    ...
    if (rank == 0) {
        printf("Write %d variables, each %dMB per proc, %d timesteps\n", n_var, size_per_proc_var_MB, n_ts);
        fflush(stdout);
    }
    ...
    if (rank == 0) { 
        printf("Write %d ts each of %.0fMB data with %d ranks: total %.2f, write %.2f, wait %.2f, sleep %d\n", 
                n_ts, total_size, size, total_time, write_time_total, wait_time_total, sleep_time*(n_ts-2));
        fflush(stdout);
    }
