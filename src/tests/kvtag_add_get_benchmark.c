          ...
          pdcid_t *cont_prop, pdcid_t *cont)
{
    ...
    *obj_prop = PDCprop_create(PDC_OBJ_CREATE, *pdc);
    ...
    assign_work_to_rank((uint64_t)my_rank, (uint64_t)proc_num, n_query, my_query, my_query_s);
           ...
           pdcid_t *obj_ids)
{
    ...
    for (i = 0; i < my_obj; i++) {
        ...
        for (j = 0; j < n_attr; j++) {
            ...
            if (PDCobj_put_tag(obj_ids[i], tag_name, (void *)tag_values[j], PDC_STRING, tag_value_len + 1) < 0)
