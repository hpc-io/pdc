...
HG_TEST_RPC_CB(metadata_index_delete, handle)
{
    ...
    HG_Get_input(handle, &in);
    // Create local index on server side
    PDC_Server_metadata_index_delete(&in, &out);
...
HG_TEST_RPC_CB(metadata_index_search, handle)
{
    ...
    PDC_Server_metadata_index_search(&in, &out, n_obj_ids_ptr, &buf_ptrs);
    // No result found
    if (*n_obj_ids_ptr == 0) {
        // *n_obj_ids_ptr = 1;
        // buf_ptrs[0]= (void*)0;
    ...
    // Create bulk handle
    hg_ret = HG_Bulk_create(hg_class_g, n_buf, buf_ptrs, buf_sizes, HG_BULK_READWRITE, &bulk_handle);
    if (hg_ret != HG_SUCCESS) {
        fprintf(stderr, "Could not create bulk data handle\n");
        return EXIT_FAILURE;
    }
