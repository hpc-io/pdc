...
void create_hash_table_for_keyword(char *keyword, char *value, size_t len, void *data){
...
int brutal_force_partial_search(metadata_query_transfer_in_t *in, uint32_t *n_meta, void ***buf_ptrs, 
    char *k_query, char *vfrom_query, char *vto_query, uint32_t *hash_value){
    int result = 0;

    uint32_t iter = 0;
    hg_hash_table_iter_t hash_table_iter;
    pdc_hash_table_entry_head *head=NULL;
    pdc_metadata_t *elt;
    int n_entry;
...
void search_through_hash_table(char *k_query, uint32_t index_type, pattern_type_t pattern_type, pdc_art_iterator_param_t *param){
...
void delete_hash_table_for_keyword(char *keyword, size_t len, void *data){
    uint32_t hashVal = djb2_hash(keyword, (int)len);
...
perr_t create_prefix_index_for_attr_value(void **index,  char *attr_value, void *data)
{
    perr_t ret = SUCCEED;
    if (*index == NULL){
...
art_tree *create_index_for_attr_name(char *attr_name, char *attr_value, void *data)
{
    ...
    int r = 0;
    for (r = 0; r < 2; r++) {
        unsigned char *val_key = (r==1? (unsigned char *)reverse_str(attr_value) :(unsigned char *) attr_value);
        secondary_trie = (r==1? (art_tree *)(leafcnt->extra_suffix_index) :(art_tree *)(leafcnt->extra_prefix_index));
...
perr_t delete_prefix_index_for_attr_value(void **index,  char *attr_value, void *data)
{
    perr_t ret = SUCCEED;
    if (*index == NULL){
...
void delete_index_for_attr_name(char *attr_name, char *attr_value, void *data){
    int len = strlen(attr_name);
    key_index_leaf_content *leaf_content = NULL;
    art_tree *nm_trie = NULL;
...
perr_t metadata_index_delete(char *attr_key, char *attr_value, uint64_t obj_locator, int8_t index_type){
    perr_t ret_value = FAIL;
    stopwatch_t timer;
    timer_start(&timer);
    ...
    timer_pause(&timer);
    println("[Server_Side_Delete_%d] Timer to delete a keyword from index = %d microseconds", pdc_server_rank_g, timer_delta_us(&timer));
...
perr_t metadata_index_create(char *attr_key, char *attr_value, uint64_t obj_locator, int8_t index_type){
    perr_t ret_value = FAIL;
    stopwatch_t timer;
    timer_start(&timer);
    ...
    timer_pause(&timer);
    println("[Server_Side_Insert_%d] Timer to insert a keyword %s : %s into index = %d microseconds", pdc_server_rank_g , attr_key, attr_value, timer_delta_us(&timer));
    indexed_word_count_g++;
    ret_value=SUCCEED;
    return ret_value;
}
 ...
 * 
 */
int level_two_art_callback(void *data, const unsigned char *key, uint32_t key_len, void *value){
 ...
 * 
 */
int level_one_art_callback(void *data, const unsigned char *key, uint32_t key_len, void *value){
        ...
        switch(level_two_ptn_type){
            case PATTERN_EXACT:
                ...
                if (leafcnt->extra_prefix_index!=NULL){
                    hashset_t obj_id_set = (hashset_t)art_search(leafcnt->extra_prefix_index, (unsigned char *)tok, strlen(tok));
                    if (obj_id_set!=NULL) {
                        level_two_art_callback((void *)param, (unsigned char*)tok, strlen(tok), (void *)obj_id_set);
                    }
                }
                break;
            case PATTERN_PREFIX:
                ...
                if (leafcnt->extra_prefix_index != NULL){
                    art_iter_prefix((art_tree *)leafcnt->extra_prefix_index, 
                    (unsigned char *)tok, strlen(tok), level_two_art_callback, param);
                }
                break;
            case PATTERN_SUFFIX:
                ...
                if (leafcnt->extra_suffix_index != NULL){
                    art_iter_prefix((art_tree *)leafcnt->extra_suffix_index, 
                    (unsigned char *)tok, strlen(tok), level_two_art_callback, param);
                }
                break;
            case PATTERN_MIDDLE:
...
perr_t metadata_index_search(char *query, int index_type, uint64_t *n_obj_ids_ptr, uint64_t ***buf_ptrs){
    ...
    if (NULL==kdelim_ptr){
        println("[Server_Side_Query_%d]query string '%s' is not valid.", 
        pdc_server_rank_g, query);
        *n_obj_ids_ptr = 0;
        return result;
    } else {
        char *tok;
        ...
        if (index_type == DHT_FULL_HASH || index_type == DHT_INITIAL_HASH){
            ...
            switch(level_one_ptn_type){
                case PATTERN_EXACT:
                    qType_string="Exact";
                    tok = k_query;
                    key_index_leaf_content *leafcnt = (key_index_leaf_content *)art_search(art_key_prefix_tree_g, 
                ...
                case PATTERN_PREFIX:
                    qType_string="Prefix";
                    tok = subrstr(k_query, strlen(k_query)-1);
                    art_iter_prefix((art_tree *)art_key_prefix_tree_g, 
                    (unsigned char *)tok, strlen(tok), level_one_art_callback, param);
                    break;
                case PATTERN_SUFFIX:
                    qType_string="Suffix";
                    tok = substr(k_query, 1);
    ...
    println("[Server_Side_%s_%d] Time to address query '%s' and get %d results  = %ld microseconds", qType_string, pdc_server_rank_g, query, *n_obj_ids_ptr , timer_delta_us(&index_timer));
