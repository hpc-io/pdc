...
client_send_transfer_request_status_rpc_cb(const struct hg_cb_info *callback_info)
{
    ...
    ret_value = HG_Get_output(handle, &output);
    if (ret_value != HG_SUCCESS) {
        printf("PDC_CLIENT[%d]: client_send_transfer_request_status_rpc_cb error with HG_Get_output\n",
               pdc_client_mpi_rank_g);
        region_transfer_args->ret = -1;
        goto done;
    }
...
client_send_transfer_request_wait_rpc_cb(const struct hg_cb_info *callback_info)
{
    ...
    if (ret_value != HG_SUCCESS) {
        printf(
            "PDC_CLIENT[%d]: client_send_transfer_request_wait_rpc_cb error with HG_Get_output @ line %d\n",
                             ...
                             pdcid_t *metadata_id, char **new_buf_ptr, pdcid_t objid, pdcid_t regid)
{
    ...
    //         access_type, mem_type, type_extent, analysis_registry[k], status);
    //       PGOTO_DONE(ret_value);
    //     }
    //   }
    // }
                                           ...
                                           pbool_t *status)
{
    ...
    hg_return_t                         hg_ret;
    uint32_t                            meta_server_id;
    transfer_request_and_transform_in_t in;
    struct _pdc_transfer_request_args   transfer_args;
    hg_handle_t                         client_send_transfer_request_handle;
    pdc_var_type_t                      data_type = mem_type;
    ...
    struct _pdc_obj_info *object_info;
    object_info        = PDC_obj_get_info(objid);
    size_t type_extent = object_info->obj_pt->type_extent;
    ...
    *data_size = client_transform_size;
    ...
    // Create bulk handle
    // hg_ret = HG_Bulk_create(hg_class, 1, (void **)&new_buf, (hg_size_t *)&total_data_size,
    // HG_BULK_READWRITE,
    // &(in.local_bulk_handle));
    hg_ret = HG_Create(send_context_g, pdc_server_info_g[data_server_id].addr, transfer_request_register_id_g,
                       &client_send_transfer_request_handle);
    if (hg_ret != HG_SUCCESS)
        PGOTO_ERROR(FAIL, "PDC_Client_transfer_request(): Could not create handle @ line %d\n", __LINE__);
    ...
    *new_buf_ptr = new_buf;
    ...
    if (client_send_transfer_request_handle != HG_HANDLE_NULL)
        HG_Destroy(client_send_transfer_request_handle);
                          ...
                          pdc_var_type_t data_type, pbool_t *status)
{
    ...
    static hg_return_t data_server_read_check_rpc_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                     ret_value = HG_SUCCESS;
        struct _pdc_client_lookup_args *client_lookup_args;
    ...
    hg_return_t PDC_Client_get_data_from_server_shm_cb(const struct hg_cb_info *callback_info)
    {
        ...
        {
            if (((pdc_metadata_t *)elt->metadata)->obj_id == read_info->obj_id &&
                elt->access_type == PDC_READ) {
...
close:
    ...
    static hg_return_t data_server_read_rpc_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                     ret_value = HG_SUCCESS;
        struct _pdc_client_lookup_args *client_lookup_args;
    ...
    static hg_return_t data_server_write_check_rpc_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t ret_value = HG_SUCCESS;
        ...
        if (ret_value != HG_SUCCESS) {
            ...
            PGOTO_ERROR(ret_value,
                        "==PDC_CLIENT[%d]: data_server_write_check_rpc_cb error with HG_Get_output",
                        pdc_client_mpi_rank_g);
        }
        client_lookup_args->ret = output.ret;
    ...
    perr_t PDC_Client_data_server_write_check(struct pdc_request * request, int *status)
    {
        ...
        hg_ret =
            HG_Forward(data_server_write_check_handle, data_server_write_check_rpc_cb, &lookup_args, &in);
        if (hg_ret != HG_SUCCESS)
            PGOTO_ERROR(FAIL, "Could not start HG_Forward()");
        ...
        *status = lookup_args.ret;
        if (lookup_args.ret != 1) {
            ...
            ret_value = SUCCEED;
            if (is_client_debug_g == 1)
                PGOTO_ERROR(FAIL, "==PDC_CLIENT[%d]: IO request not done by server yet",
                            pdc_client_mpi_rank_g);
    ...
    static hg_return_t data_server_write_rpc_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                     ret_value = HG_SUCCESS;
        struct _pdc_client_lookup_args *client_lookup_args;
        hg_handle_t                     handle;
    ...
    perr_t PDC_Client_data_server_write(struct pdc_request * request)
    {
        ...
        if (request->shm_base == MAP_FAILED)
            PGOTO_ERROR(FAIL, "==PDC_CLIENT: Shared memory mmap failed, region size = %" PRIu64 "",
                        region_size);
    ...
    perr_t PDC_Client_iwrite(pdc_metadata_t * meta, struct pdc_region_info * region,
                             struct pdc_request * request, void *buf)
    {
        perr_t ret_value = SUCCEED;
    ...
    perr_t PDC_Client_iread(pdc_metadata_t * meta, struct pdc_region_info * region,
                            struct pdc_request * request, void *buf)
    {
        perr_t ret_value = SUCCEED;
    ...
    perr_t PDC_Client_write_wait_notify(pdc_metadata_t * meta, struct pdc_region_info * region, void *buf)
    {
        ...
        ret_value = PDC_Client_iwrite(meta, region, request, buf);
        if (ret_value != SUCCEED)
            PGOTO_ERROR(FAIL, "==PDC_CLIENT[%d]: Failed to send write request to server",
                        pdc_client_mpi_rank_g);
    ...
    perr_t PDC_Client_read_wait_notify(pdc_metadata_t * meta, struct pdc_region_info * region, void *buf)
    {
        ...
        ret_value = PDC_Client_iread(meta, region, request, buf);
        if (ret_value != SUCCEED)
            PGOTO_ERROR(FAIL, "==PDC_CLIENT[%d]: Faile to send read request to server",
                        pdc_client_mpi_rank_g);
    ...
    static hg_return_t PDC_Client_add_del_objects_to_container_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                             ret_value = HG_SUCCESS;
        hg_handle_t                             handle    = callback_info->info.forward.handle;
    ...
    perr_t PDC_Client_add_del_objects_to_container(int nobj, uint64_t *obj_ids, uint64_t cont_meta_id, int op)
    {
        ...
        /* Register memory */
        hg_ret =
            HG_Bulk_create(send_class_g, 1, (void **)&obj_ids, buf_sizes, HG_BULK_READ_ONLY, &bulk_handle);
        if (hg_ret != HG_SUCCESS)
    ...
    perr_t PDC_Client_add_tags_to_container(pdcid_t cont_id, char *tags)
    {
        ...
        if (info == NULL)
            PGOTO_ERROR(FAIL, "==PDC_CLIENT[%d]: cont_id %" PRIu64 " invalid!", pdc_client_mpi_rank_g,
                        cont_id);
    ...
    static hg_return_t container_query_rpc_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                       ret_value;
    ...
    static hg_return_t PDC_Client_query_read_obj_name_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                             ret_value = HG_SUCCESS;
        hg_handle_t                             handle    = callback_info->info.forward.handle;
        pdc_int_ret_t                           bulk_rpc_ret;
                                                 ...
                                                 uint64_t *out_buf_sizes)
    {
        ...
        /* Register memory */
        hg_ret = HG_Bulk_create(send_class_g, nobj, (void **)obj_names, buf_sizes, HG_BULK_READ_ONLY,
                                &bulk_handle);
        if (hg_ret != HG_SUCCESS)
            PGOTO_ERROR(FAIL, "Could not create bulk data handle");
    ...
    perr_t PDC_Client_complete_read_request(int nbuf, struct pdc_request *req)
    {
        ...
        *req->buf_arr     = (void **)calloc(nbuf, sizeof(void *));
        ...
        for (i = 0; i < nbuf; i++) {
            ...
            /* map the shared memory segment to the address space of the process */
            req->shm_base_arr[i] =
                mmap(0, (req->shm_size_arr)[i], PROT_READ, MAP_SHARED, req->shm_fd_arr[i], 0);
    ...
    perr_t PDC_Client_server_checkpoint(uint32_t server_id)
    {
        ...
        hg_ret = HG_Create(send_context_g, pdc_server_info_g[server_id].addr,
                           server_checkpoint_rpc_register_id_g, &rpc_handle);
        if (hg_ret != HG_SUCCESS)
    ...
    */
    ...
    perr_t PDC_send_region_storage_meta_shm(uint32_t server_id, int n, region_storage_meta_t *storage_meta)
    {
        ...
        cb_args.rpc_handle  = rpc_handle;
        hg_ret =
            HG_Forward(rpc_handle, /* reuse */ PDC_Client_query_read_obj_name_cb, &cb_args, &bulk_rpc_in);
        if (hg_ret != HG_SUCCESS)
            PGOTO_ERROR(FAIL, "Could not forward call");
    ...
    perr_t PDC_Client_cp_data_to_local_server(int nobj, region_storage_meta_t **all_storage_meta,
                                              void ***buf_arr, size_t *size_arr)
    ...
    perr_t PDC_Client_read_with_storage_meta(int nobj, region_storage_meta_t **all_storage_meta,
                                             void ***buf_arr, size_t *size_arr)
        ...
        *buf_arr = (void **)calloc(sizeof(void *), nobj);
        ...
        for (i = 0; i < nobj; i++) {
            ...
            if (cache_count_g < cache_total_g) {
                cache_region =
                    PDC_get_storage_meta_from_io_list(&client_cache_list_head_g, all_storage_meta[i]);
    ...
    perr_t PDC_Client_query_multi_storage_info(int nobj, char **obj_names,
                                               region_storage_meta_t ***all_storage_meta)
    {
        ...
        for (iter = 0; iter < pdc_server_num_g; iter++) {
            ...
            for (j = 0; j < ((process_bulk_storage_meta_args_t *)request->storage_meta)->n_storage_meta;
                 j++) {
                                                        ...
                                                        uint64_t *out_buf_sizes)
    {
        ...
        ret_value =
            PDC_Client_read_with_storage_meta(nobj, all_storage_meta, out_buf, (size_t *)out_buf_sizes);
                                                            ...
                                                            size_t *out_buf_sizes)
    {
        ...
        // Gather all object names to rank 0 of each node
        MPI_Gatherv(local_names_1d, my_nobj * max_name_len, MPI_CHAR, all_names_1d, recvcounts, displs,
                    MPI_CHAR, 0, PDC_SAME_NODE_COMM_g);
        ...
        if (pdc_client_same_node_rank_g == 0) {
            ...
            // res_storage_meta_1d = (region_storage_meta_t *)calloc(sizeof(region_storage_meta_t),
            // ntotal_obj);
        ...
        if (cache_percentage_g == 100)
            ret_value =
                PDC_Client_cp_data_to_local_server(ntotal_obj, my_storage_meta, out_buf, out_buf_sizes);
                                           ...
                                           uint64_t file_offset, void *buf, size_t *total_read_bytes)
    {
        ...
        uint64_t buf_offset = 0, storage_offset = file_offset, total_bytes = 0, read_bytes = 0,
                 row_offset = 0;
        uint64_t i = 0, j = 0;
        ...
        *total_read_bytes = 0;
        if (ndim > 3 || ndim <= 0)
            PGOTO_ERROR(FAIL, "==PDC_CLIENT[%d]: dim=%" PRIu32 " unsupported yet!", pdc_client_mpi_rank_g,
                        ndim);
                ...
                for (j = 0; j < overlap_count[2]; j++) {
                    ...
                    for (i = 0; i < overlap_count[1]; i++) {
                        ...
                        buf_serialize_offset =
                            buf_offset + i * req_count[0] + j * req_count[0] * req_count[1];
                        if (is_client_debug_g == 1) {
    ...
    perr_t PDC_Client_query_name_read_entire_obj_client_agg_cache_iter(
        int my_nobj, char **my_obj_names, void ***out_buf, size_t *out_buf_sizes, int cache_percentage)
    {
        perr_t ret_value   = SUCCEED;
    ...
    static hg_return_t metadata_get_kvtag_rpc_cb(const struct hg_cb_info *callback_info)
    {
    ...
    static hg_return_t kvtag_query_bulk_cb(const struct hg_cb_info *hg_cb_info)
    {
        hg_return_t         ret_value = HG_SUCCESS;
        struct bulk_args_t *bulk_args;
    ...
    static hg_return_t kvtag_query_forward_cb(const struct hg_cb_info *callback_info)
    {
        hg_return_t                   ret_value;
    ...
    static perr_t PDC_Client_query_kvtag_server(uint32_t server_id, const pdc_kvtag_t *kvtag, int *n_res,
                                                uint64_t **out)
    {
        perr_t              ret_value = SUCCEED;
        hg_return_t         hg_ret;
    ...
    // Get container name
    perr_t PDCcont_get(pdcid_t cont_id ATTRIBUTE(unused), char **cont_name ATTRIBUTE(unused))
    {
        perr_t ret_value = SUCCEED;
    ...
    perr_t PDCcont_get_objids(pdcid_t cont_id ATTRIBUTE(unused), int *nobj ATTRIBUTE(unused),
    ...
    perr_t PDC_Client_del_metadata(pdcid_t obj_id, int is_cont)
    {
        perr_t                 ret_value = SUCCEED;
        uint64_t               meta_id;
        struct _pdc_obj_info * obj_prop;
    ...
    perr_t PDC_send_data_query(pdc_query_t * query, pdc_query_get_op_t get_op, uint64_t * nhits,
                               pdc_selection_t * sel, void *data       ATTRIBUTE(unused))
    {
        perr_t                         ret_value      = SUCCEED;
        hg_return_t                    hg_ret         = 0;
        ...
        for (server_id = 0; server_id < pdc_server_num_g; server_id++) {
            ...
            HG_Create(send_context_g, pdc_server_info_g[server_id].addr, send_data_query_register_id_g,
                      &handle);
