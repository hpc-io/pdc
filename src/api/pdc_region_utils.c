    ...
    *output_size   = *output_offset + ndim;
    for (i = 0; i < ndim; ++i) {
        ...
        output_size[0][i]   = ((offset2[i] + size2[i] < offset1[i] + size1[i]) ? (offset2[i] + size2[i])
                                                                             : (offset1[i] + size1[i])) - output_offset[0][i];
    }
    ...
    switch(ndim) {
        ...
        case 2: {
            for (i = 0; i < overlap_size[0]; ++i) {
                src_buf = buf + ((overlap_offset[0] - offset[0] + i) * size[1] + overlap_offset[1] - offset[1]) * unit;
                target_buf    = buf2 + ((overlap_offset[0] - offset2[0] + i) * size2[1] + overlap_offset[1] - offset2[1]) * unit;
        ...
        case 3: {
            for (i = 0; i < overlap_size[0]; ++i) {
                for (j = 0; j < overlap_size[1]; ++j) {
                    src_buf = buf + ( ((overlap_offset[0] - offset[0] + i) * size[1]  + (overlap_offset[1] - offset[1] + j)) * size[2] + overlap_offset[2] - offset[2]) * unit;
                    target_buf = buf2 + ( ((overlap_offset[0] - offset2[0] + i) * size2[1]  + (overlap_offset[1] - offset2[1] + j)) * size2[2] + overlap_offset[2] - offset2[2]) * unit;
