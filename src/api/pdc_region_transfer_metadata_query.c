...
static perr_t transfer_request_metadata_reg_append(pdc_region_metadata_pkg *regions, int ndim, uint64_t *reg_offset, uint64_t *reg_size, size_t unit);
static uint64_t transfer_request_metadata_query_append(uint64_t obj_id, int ndim, uint64_t *reg_offset, uint64_t *reg_size, size_t unit);
static uint64_t metadata_query_buf_create(pdc_obj_region_metadata *regions, int size, uint64_t *total_buf_size_ptr);
    ...
    if (checkpoint) {
        ...
        for ( i = 0; i < n_objs; ++i ) {
            if ( metadata_server_objs ) {
                metadata_server_objs_end->next = (pdc_obj_metadata_pkg *) malloc(sizeof(pdc_obj_region_metadata));
            ...
            metadata_server_objs_end->regions = (pdc_region_metadata_pkg *) malloc(sizeof(pdc_region_metadata_pkg));
            metadata_server_objs_end->regions_end = metadata_server_objs_end->regions;

            metadata_server_objs_end->regions_end->next = NULL;
            metadata_server_objs_end->regions_end->reg_offset = (uint64_t *) malloc(sizeof(uint64_t) * metadata_server_objs_end->ndim * 2);
            metadata_server_objs_end->regions_end->reg_size = metadata_server_objs_end->regions_end->reg_offset + metadata_server_objs_end->ndim;
            metadata_server_objs_end->regions_end->data_server_id = *(uint32_t*) ptr;
            ptr += sizeof(uint32_t);
            memcpy(metadata_server_objs_end->regions_end->reg_offset, ptr, sizeof(uint64_t) * metadata_server_objs_end->ndim * 2);
            ptr += sizeof(uint64_t) * metadata_server_objs_end->ndim * 2;

            for ( j = 1; j < reg_count; ++j ) {
                metadata_server_objs_end->regions->next = (pdc_region_metadata_pkg *) malloc(sizeof(pdc_region_metadata_pkg));
...
static uint64_t metadata_query_buf_create(pdc_obj_region_metadata *regions, int size, uint64_t *total_buf_size_ptr) {
    ...
    for ( i = 0; i < size; ++i ) {
        ...
        if ( temp ) {
            ...
            while (region_metadata) {
                if ( check_overlap(regions[i].ndim, region_metadata->reg_offset, region_metadata->reg_size, regions[i].reg_offset, regions[i].reg_size) ) {
            ...
            total_data_size += sizeof(int) + transfer_request_counters[i] * (sizeof(uint32_t) + sizeof(uint64_t) * regions[i].ndim * 2);
        } else {
    ...
    for ( i = 0; i < size; ++i ) {
        ...
        if ( temp ) {
            region_metadata = temp->regions;
            while (region_metadata) {
                PDC_region_overlap_detect(regions[i].ndim, region_metadata->reg_offset, region_metadata->reg_size, regions[i].reg_offset, regions[i].reg_size, &overlap_offset, &overlap_size);
                if ( overlap_offset ) {
 ...
 * Output: query ID, the query entry contains obj ID, data server ID and overlapping region (client can directly use these information to forward its requests)
*/
uint64_t transfer_request_metadata_query_parse(int32_t n_objs, char *buf, uint8_t is_write, uint64_t *total_buf_size_ptr) {
    char *ptr = buf;
    int i;
    uint64_t query_id = 0;
    size_t unit;
    pdc_obj_region_metadata *region_metadata = (pdc_obj_region_metadata*) malloc(sizeof(pdc_obj_region_metadata) * n_objs);
    ...
    for ( i = 0; i < n_objs; ++i ) {
        ...
        ptr += sizeof(uint64_t) * region_metadata[i].ndim;
        if ( is_write ) {
            transfer_request_metadata_query_append(region_metadata[i].obj_id, region_metadata[i].ndim, region_metadata[i].reg_offset, region_metadata[i].reg_size, unit);
        }
    }
...
static uint64_t transfer_request_metadata_query_append(uint64_t obj_id, int ndim, uint64_t *reg_offset, uint64_t *reg_size, size_t unit) {
    ...
    region_metadata = temp->regions;
    while ( region_metadata ) {
        if ( detect_region_contained(reg_offset, reg_size, region_metadata->reg_offset, region_metadata->reg_size, ndim) ) {
            printf("%lu, %lu, %lu ,%lu\n", reg_offset[0], reg_size[0], region_metadata->reg_offset[0], region_metadata->reg_size[0]);
