                ...
                for (int j = 0; j < numReg; j++) {
                    ...
                    for (int dim3 = 0; dim3 < count_z; dim3++) {
                        ...
                        for (int dim2 = 0; dim2 < count_y; dim2++) {
                            ...
                            // new_region_info->count_0[i]*in.data_unit);
                            memcpy(to_be_written + all_cur_offset, orig_file,
                                   new_region_info->count_0[i] * in.data_unit);
                            all_cur_offset += new_region_info->count_0[i] * in.data_unit;
                        }
                    }
                    ...
                    sprintf(insertSql,
                            "INSERT INTO Objects (TimeStampSec, TimeStampUsec, Date, IOType, ProgName, "
                            "ObjectID, ndim, start_x, start_y, start_z, count_x, count_y, count_z, dim_x, "
                            "dim_y, dim_z, offset, unit, sizeReg, totalNumRegs, totalBytes, newObjFilePath) "
                            "VALUES (%d, %d, DATETIME('now'), 'READ', 'VPICIO', %d, %d, %" PRIu64 ", %" PRIu64
                            ", %" PRIu64 ", %" PRIu64 ", %" PRIu64 ", %" PRIu64 ", %" PRIu64 ", %" PRIu64
                            ", %" PRIu64 ", %" PRIu64 ", %d, %" PRIu64 ", %d, %" PRIu64 ", %s);",
                            prefetch_ts->tv_sec, prefetch_ts->tv_usec, objId, ndim, regsStart_x[j],
                            regsStart_y[j], regsStart_z[j], count_x, count_y, count_z, dim_x, dim_y, dim_z,
                            all_buf_offset, unit, count_x * count_y * count_z * unit, numReg, totalBytes,
