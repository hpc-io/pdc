...
PDCregion_transfer_start(pdcid_t transfer_request_id)
{
    ...
    if (transfer_request->region_partition == PDC_REGION_STATIC) {
        ...
        */
        ...
        for (i = 0; i < transfer_request->n_obj_servers; ++i) {
            ...
            //     transfer_request->output_buf[i], transfer_request->obj_id,
            ...
            ret_value = PDC_Client_transfer_request2(
                ...
                transfer_request->remote_region_ndim, transfer_request->output_offsets[i],
                transfer_request->output_sizes[i], transfer_request->mem_type, unit,
    ...
    if (transfer_request->region_partition == PDC_OBJ_STATIC) {
        ...
        ret_value = PDC_Client_transfer_request2(
            ...
            transfer_request->remote_region_size, transfer_request->mem_type, unit,
            transfer_request->access_type, transfer_request->metadata_id, &(transfer_request->new_buf),
            transfer_request->objid, transfer_request->regid);
    }
done:
    fflush(stdout);
    FUNC_LEAVE(ret_value);
