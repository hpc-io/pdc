...
static hg_id_t          metadata_index_delete_g;
static hg_id_t         metadata_index_search_g;
...
// #ifndef PDC_HAS_CRAY_DRC
    ...
    *hg_context = HG_Context_create(*hg_class);
    ...
    // metadata_index_delete_g                   = metadata_index_delete_register(*hg_class);
    // metadata_index_search_g                   = metadata_index_search_register(*hg_class);
    dart_get_server_info_g                    = dart_get_server_info_register(*hg_class);
    dart_perform_one_server_g                 = dart_perform_one_server_register(*hg_class);
...
perr_t server_lookup_connection(int serverId, int retry_times){
    ...
    while (pdc_server_info_g[serverId].addr_valid != 1) {
        if (n_retry > retry_times) 
            break;
        if( PDC_Client_lookup_server(serverId) == SUCCEED) {
            rst = SUCCEED;
            break;
        } else {
            rst = FAIL;
            printf("==CLIENT[%d]: ERROR with PDC_Client_lookup_server, retry = %d\n", pdc_client_mpi_rank_g, n_retry);
        }
        n_retry++;
    }
    return rst;
...
dart_server dart_retrieve_server_info_cb(uint32_t serverId){
    ...
    HG_Create(send_context_g, pdc_server_info_g[serverId].addr, dart_get_server_info_g, &dart_get_server_info_handle);
...
int dart_perform_on_one_server(int server_id, dart_perform_one_server_in_t *dart_in, hashset_t *hashset){
    ...
    HG_Create(send_context_g, pdc_server_info_g[server_id].addr, dart_perform_one_server_g, &dart_perform_one_server_handle);
    ...
    hg_ret = HG_Forward(dart_perform_one_server_handle, dart_perform_one_server_on_receive_cb, &lookup_args, dart_in);
...
perr_t PDC_Client_search_obj_ref_through_dart(dart_hash_algo_t hash_algo, char *query_string, 
    ...
    *out = NULL;
    ...
    if (hash_algo == DART_HASH) {
        num_servers = DART_hash(dart_g, tok, dart_query_type, NULL, &server_id_arr);
    } else if (hash_algo == DHT_FULL_HASH) {
        num_servers = DHT_hash(dart_g, strlen(tok), tok, dart_query_type, &server_id_arr);
    } else if (hash_algo == DHT_INITIAL_HASH) {
        num_servers = DHT_hash(dart_g, 1, tok, dart_query_type, &server_id_arr);
    }

    // Determine whether we should omit some requests:
    int omit_request = 0;
    if (dart_query_type == PATTERN_PREFIX || 
        dart_query_type == PATTERN_SUFFIX) {
        if (strlen(tok) >= dart_g->dart_tree_height && 
            num_servers == 2) {
            omit_request = 1;
        }
    }
    ...
    for (i = 0; i < num_servers; i++){
        ...
        // dart_perform_one_thread_param_t *thread_param = (dart_perform_one_thread_param_t *)calloc(1, 
        // sizeof(dart_perform_one_thread_param_t));
        // thread_param->server_id = serverId;
        ...
        *out = (uint64_t**)calloc(*n_res, sizeof(uint64_t*));
        ...
        while (i < hashset->nitems && itr_idx < hashset->capacity){
            if (hashset->items[itr_idx] != 0) {
                ((uint64_t *)(*out)[i])[0] = hashset->items[itr_idx];
                i++;
            }
            itr_idx++;
        }
