...
threadpool get_dart_temp_thpool(int count){
...
void dart_space_init(DART *dart, int num_client, int num_server, 
    int alphabet_size, int extra_tree_height, int replication_factor){
    if (dart == NULL) {
 ...
 * 
 */
uint64_t get_reconciled_vnode_id_with_power_of_two_choice_rehashing(DART *dart, 
    uint64_t base_vnode_idx, char *word, 
    ...
    if (strlen(word) <= tree_height) {
        ...
        if (strlen(word) <= 1){
            // if the word contains 0-1 character, there is no pre-leaf character and therefore pre_leaf_index should be 0.
            pre_leaf_index = 0;
        } else {
    ...
    // We calcuate the minor offset which will possibly pick a different virtual node within the same sub-region.
    ...
    // uint64_t region_start = ((((int)word[0]+ir_idx) % dart->alphabet_size)) * region_size;// ((reconciled_vnode_idx)/region_size) * (region_size);
 ...
 * 
 */
uint64_t get_reconciled_vnode_id_with_power_of_two_choice_rehashing_2(DART *dart, 
    uint64_t base_vnode_idx, char *word, 
    get_server_info_callback get_server_cb){
 ...
 * \param get_server_cb [IN]            A callback function which determines the lighter-load server between two servers.
 * \param out           [OUT]           The pointer of the server ID array.
 * \return                              The length of the server ID.
 */
int
get_server_ids_for_insert(DART *dart_g, char *keyword, 
get_server_info_callback get_server_cb, uint64_t **out){
    if (out == NULL) {
        return 0;    
    }
 ...
 * 
 * \param dart_g        [IN]        Pointer of Global state of DART
 * \param token         [IN]        The token which will searched for. It should not contain any wildcard sign. 
 * \param op_type       [IN]        Give the operation type.
 * \param out           [IN/OUT]    The pointer of the server ID array. To accelerate, we require caller function to pass an ID array of length M into this function,
 *                                  where M is the number of total physical servers. Then, this function will fill up this array.
...
get_server_ids_for_query(DART *dart_g, char *token, dart_op_type_t op_type, uint64_t **out){
    ...
    if (op_type == OP_INSERT){return 0;} // For INSERT operation ,we return nothing here.
 ...
 * \param op_type       [IN]        Give the operation type.
 * \param get_server_cb [IN]        The callback function for getting the statistical information of a server. Signature: dart_server (*get_server_info_callback)(uint32_t server_id)
 * \param out           [IN/OUT]    The pointer of the server ID array. To accelerate, we require caller function to pass an ID array of length M into this function,
 *                                  where M is the number of total physical servers. Then, this function will fill up this array.
 * \return              [OUT]       The valid length of the server ID array.
 * 
 */
int DART_hash(DART *dart_g, char *key, dart_op_type_t op_type, 
                get_server_info_callback get_server_cb, uint64_t **out){
    if (out == NULL) {
        return 0;    
    }
