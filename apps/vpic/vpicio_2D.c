...
int main(int argc, char **argv)
{
	...
	x=(float*)malloc(numparticles*c*sizeof(double));
        y=(float*)malloc(numparticles*c*sizeof(double));
        z=(float*)malloc(numparticles*c*sizeof(double));

        px=(float*)malloc(numparticles*c*sizeof(double));
        py=(float*)malloc(numparticles*c*sizeof(double));
        pz=(float*)malloc(numparticles*c*sizeof(double));

        id1=(int*)malloc(numparticles*c*sizeof(int));
        id2=(int*)malloc(numparticles*c*sizeof(int));
	...
	for (i=0; i<numparticles; i++)
		...
		for(j=0; j<c; j++)
		{
                *(id1+i*c+j) = i;
                *(id2+i*c+j) = i*2;
                *(x+i*c+j) = uniform_random_number() * x_dim;
                *(y+i*c+j) = uniform_random_number()*y_dim;
                *(z+i*c+j) = ((double)id1[i]/numparticles)*z_dim;
                *(px+i*c+j) = uniform_random_number()*x_dim;
                *(py+i*c+j) = uniform_random_number()*y_dim;
                *(pz+i*c+j) = ((double)id2[i]/numparticles)*z_dim;
        	}
	}
    
    const int my_data_size = 8388608;
    uint64_t dims[2]={my_data_size,c};  // {8388608};
