...
FILE *fp1_g;
double size_b_g = 0;
double size_min_g = 2148473647, size_max_g = 0;
int n_dset = 0;
...
main(int argc, char **argv)
{
    ...
    if (argc != 4)
        printf("\nPlease specify an HDF5 file.\nUsage: %s file output_dset output_summary\n", argv[0]);
    else {
       filename = argv[1];
       outputname = argv[2];
       output_summary = argv[3];

       // Append output file 
       fp_g = fopen(outputname, "a");
       if (fp_g == NULL) {
           printf("\nOutput file [%s] open error!\n", outputname);
       }

       // Append output file 
       fp1_g = fopen(output_summary, "a");
       if (fp1_g == NULL) {
           printf("\nOutput file [%s] open error!\n", output_summary);
       }
       ...
       for (i = strlen(filename); i > 0; i--) {
           if (filename[i] == '/') {
               str_ptr = &filename[i+1];
               break;
           }
       }
       fprintf(fp1_g, "%s, %d, %.0f, %.0f, %.0f, %.0f\n", 
               str_ptr, n_dset, size_b_g, size_min_g, size_b_g/n_dset, size_max_g);
       fclose(fp_g);
       fclose(fp1_g);
       return 0;
    }
...
do_dset(hid_t did)
{
	...
	tid = H5Dget_type(did);
        ...
        int i;
        fprintf(fp_g, "%dD:", ndims);
        H5Sget_simple_extent_dims(sid, dims, NULL);

        for (i = 0; i < ndims; i++) {
            fprintf(fp_g, " %lu", dims[i]);
        }
	...
	size = H5Dget_storage_size(did);
        double size_b;
        size_b = size;
        /* size_b = size / 1048576.0; */
	/* printf("Total space currently written in file: %d\n",(int)size); */
        size_b_g += size_b;
        if (size_b < size_min_g) 
            size_min_g = size_b;

        if (size_b > size_max_g) 
            size_max_g = size_b;
